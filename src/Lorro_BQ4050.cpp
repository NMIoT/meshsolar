...
#define PRINTBIN(Num) for (uint64_t t = (1ULL<< ((sizeof(Num)*8)-1)); t; t >>= 1) Serial2.write(Num  & t ? '1' : '0');

boolean Lorro_BQ4050::getOperationStatus(){
  if( readBlockReg( registers.operationStatus ) ){
    Serial2.println( "Operation status bits:" );
    for( int i = 0; i < 4; i++ ){
      for( int j = 0; j < 8; j++ ){
        Serial2.print( "(" );
        Serial2.print( ( registers.operationStatus.val[ i ] & ( 0x01 << ( 7 - j ) ) ) >> ( 7 - j ) );
        Serial2.print( ") " );
        Serial2.print( registers.operationStatus.label[ i ][ j ] );
        Serial2.print( "\t" );
      }
      Serial2.println();
    }
...
boolean Lorro_BQ4050::getChargingStatus(){
  if( readBlockReg( registers.chargingStatus ) ){
    Serial2.println( "Charging status bits:" );
    for( int i = 0; i < 2; i++ ){
      for( int j = 0; j < 8; j++ ){
        Serial2.print( "(" );
        Serial2.print( ( registers.chargingStatus.val[ i ] & ( 0x01 << ( 7 - j ) ) ) >> ( 7 - j ) );
        Serial2.print( ") " );
        Serial2.print( registers.chargingStatus.label[ i ][ j ] );
        Serial2.print( "\t" );
      }
      Serial2.println();
    }
...
boolean Lorro_BQ4050::getGaugingStatus(){
  if( readBlockReg( registers.gaugingStatus ) ){
    Serial2.println( "Gauging status bits:" );
    for( int i = 0; i < 2; i++ ){
      for( int j = 0; j < 8; j++ ){
        Serial2.print( "(" );
        Serial2.print( ( registers.gaugingStatus.val[ i ] & ( 0x01 << ( 7 - j ) ) ) >> ( 7 - j ) );
        Serial2.print( ") " );
        Serial2.print( registers.gaugingStatus.label[ i ][ j ] );
        Serial2.print( "\t" );
      }
      Serial2.println();
    }
...
boolean Lorro_BQ4050::getSafetyStatus(){
  if( readBlockReg( registers.safetyStatus ) ){
    Serial2.println( "Safety status bits:" );
    for( int i = 0; i < 4; i++ ){
      for( int j = 0; j < 8; j++ ){
        Serial2.print( "(" );
        Serial2.print( ( registers.safetyStatus.val[ i ] & ( 0x01 << ( 7 - j ) ) ) >> ( 7 - j ) );
        Serial2.print( ") " );
        Serial2.print( registers.safetyStatus.label[ i ][ j ] );
        Serial2.print( "\t" );
      }
      Serial2.println();
    }
...
boolean Lorro_BQ4050::getSafetyAlert(){
  if( readBlockReg( registers.safetyAlert ) ){
    Serial2.println( "Safety alert bits:" );
    for( int i = 0; i < 4; i++ ){
      for( int j = 0; j < 8; j++ ){
        Serial2.print( "(" );
        Serial2.print( ( registers.safetyAlert.val[ i ] & ( 0x01 << ( 7 - j ) ) ) >> ( 7 - j ) );
        Serial2.print( ") " );
        Serial2.print( registers.safetyAlert.label[ i ][ j ] );
        Serial2.print( "\t" );
      }
      Serial2.println();
    }
...
boolean Lorro_BQ4050::getXCHGstatus(){
  if( readBlockReg( registers.operationStatus ) ){
    ...
    if( ( ( registers.operationStatus.val[ 1 ] & 0x40 ) >> 6 ) == 1 ){
      Serial2.println("Charging is disabled \t\t\t(XCHG high)");
    }else{
      Serial2.println("Charging is not disabled \t\t(XCHG low)");
    }
    return true; //return if the bit read has been successful
  }else{
    return false;
...
//Function to read the register and extract the status bit for human reading
  ...
  if( readBlockReg( registers.operationStatus ) ){
    ...
    if( ( ( registers.operationStatus.val[ 1 ] & 0x20 ) >> 5 ) == 1 ){
      Serial2.println("Discharging is disabled \t(XDSG high)");
    }else{
      Serial2.println("Discharging is not disabled \t\t(XDSG low)");
    }
    return true; //return if the bit read has been successful
  }else{
    return false;
...
boolean Lorro_BQ4050::getPCHGstatus(){
  if( readBlockReg( registers.operationStatus ) ){
    ...
    if( ( ( registers.operationStatus.val[ 0 ] & 0x08 ) >> 3 ) == 1 ){
      Serial2.println("Pre-charge FET is enabled (on) \t\t(PCHG high)");
    }else{
      Serial2.println("Pre-charge FET is disabled (off) \t(PCHG low)");
    }
    return true; //return if the bit read has been successful
  }else{
    return false;
...
boolean Lorro_BQ4050::getCHGstatus(){
  if( readBlockReg( registers.operationStatus ) ){
    ...
    if( ( ( registers.operationStatus.val[ 0 ] & 0x04 ) >> 2 ) == 1 ){
      Serial2.println("Charging FET is enabled (on) \t\t(CHG high)");
    }else{
      Serial2.println("Charging FET is disabled (off) \t\t(CHG low)");
    }
    return true; //return if the bit read has been successful
  }else{
    return false;
...
boolean Lorro_BQ4050::getDSGstatus(){
  if( readBlockReg( registers.operationStatus ) ){
    ...
    if( ( ( registers.operationStatus.val[ 0 ] & 0x02 ) >> 1 ) == 1 ){
      Serial2.println("Discharging FET is enabled (on) \t(DSG high)");
    }else{
      Serial2.println("Discharging FET is disabled (off) \t(DSG low)");
    }
    return true; //return if the bit read has been successful
  }else{
    return false;
...
boolean Lorro_BQ4050::getCellBalancingStatus(){
  if( readBlockReg( registers.gaugingStatus ) ){
    ...
    if( ( ( registers.gaugingStatus.val[ 0 ] & 0x10 ) >> 4 ) == 1 ){
      Serial2.println("Cell balancing is enabled (on) \t\t(BAL_EN high)");
    }else{
      Serial2.println("Cell balancing is disabled (off) \t(BAL_EN low)");
    }
    return true; //return if the bit read has been successful
  }else{
    return false;
...
boolean Lorro_BQ4050::numberOfCells( uint8_t cellNum ){
  ...
  Serial2.print( "DA config byte from cpp: " );
  Serial2.println( dataFlash.settings.configuration.dAConfiguration.val, HEX );
...
void Lorro_BQ4050::getHWversion(){
  // 2. 读取BlockAccess寄存器
  struct {
      uint8_t val[4];
      uint8_t addr = 0x44;
  ...
  writeMACommand(BQ4050ADDR, 0x0002); 
  if (readBlockReg(hwver_block)) {
    uint16_t hwver = hwver_block.val[2] | (hwver_block.val[3] << 8); // Little Endian
    Serial2.print("Hardware Version: ");
    Serial2.println(hwver, HEX);
  } else {
    Serial2.println("Read hardware version failed!");
  }
}
...
boolean Lorro_BQ4050::writeMACommand( char devAddress, byte regAddress ){
  ...
  Wire.write( PECcheck );
  byte ack = Wire.endTransmission();
  return ( ack == 0 ); //return true if ack received, false if not
}
...
//Function that generates byte array as a lookup table to quickly create a CRC8 for the PEC
  ...
  for (int divident = 0; divident < 256; divident++){
    ...
    if( printResults ){
      ...
      if( currByte < 16 ) Serial2.print( "0" );
      Serial2.print( currByte, HEX );
      Serial2.print( "\t" );
    }
  }
  if( printResults ){
...
byte Lorro_BQ4050::Compute_CRC8(byte *bytes, uint8_t byteLen){
  ...
  for( int i = 0; i < byteLen; i++ ){
    ...
    if( printResults ){
      Serial2.print( "byte value: " );
      Serial2.print( bytes[i], HEX );
      Serial2.print( "\tlookup position: " );
      Serial2.print( data, HEX );
      Serial2.print( "\tlookup value: " );
      Serial2.println( crc, HEX );
    }
  }
