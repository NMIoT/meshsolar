...
bool BQ4050::rd_mac_block(uint8_t *data, uint8_t arrLen) {
    uint8_t buf[32] = {
        (uint8_t)(this->devAddr<<1),     
        BLOCK_ACCESS_CMD, 
        (uint8_t)((this->devAddr<<1) + 1),
    };
    ...
    this->wire->requestFrom((uint8_t)this->devAddr, (uint8_t)(arrLen + 2)); // send start signal
